...
public class LinearSmoothScroller extends RecyclerView.SmoothScroller {
     ...
     *
     * recyclerview point left-top[?,boxStart] right-top[?,boxstart] left-bottom[?,boxEnd] right-bottom[?,boxEnd]
     * childview point left-top[?,viewStart] right-top[?,viewStart] left-bottom[?,viewEnd] right-bottom[?,viewEnd]
     */
    public int calculateDtToFit(int viewStart, int viewEnd, int boxStart, int boxEnd, int
            snapPreference) {
        Log.d("lzqtest","LinearSmoothScroller.java.calculateDtToFit: viewStart= "+viewStart+" viewEnd="+viewEnd+" boxStart="+boxStart+" boxEnd="+boxEnd);
        switch (snapPreference) {
            case SNAP_TO_START:
                Log.d("lzqtest","LinearSmoothScroller.java.calculateDtToFit: boxStart - viewStart= "+(boxStart - viewStart));
                return boxStart - viewStart;
            case SNAP_TO_END:
                Log.d("lzqtest","LinearSmoothScroller.java.calculateDtToFit: boxEnd - viewEnd= "+(boxEnd - viewEnd));
                return boxEnd - viewEnd;
            case SNAP_TO_ANY:
                final int dtStart = boxStart - viewStart;
                if (dtStart > 0) {
                    Log.d("lzqtest","LinearSmoothScroller.java.calculateDtToFit: 283 dtStart = boxStart - viewStart="+dtStart);
                    return dtStart;
                }
                final int dtEnd = boxEnd - viewEnd;
                if (dtEnd < 0) {
                    Log.d("lzqtest","LinearSmoothScroller.java.calculateDtToFit: dtEnd = boxEnd - viewEn= "+(boxEnd - viewEnd));
                    return dtEnd;
                }
                break;
            default:
